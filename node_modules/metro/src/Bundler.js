...
class Bundler {
  ...
  getOptions() {
    ...
    return _asyncToGenerator(function*() {
      ...
      yield _this._depGraphPromise.then(function(dependencyGraph) {
        return dependencyGraph.getWatcher().end();
      });
    })();
  }
   ...
   */
  getTransformOptionsForEntryFiles(entryFiles, options, getDependencies) {
    var _this2 = this;
    return _asyncToGenerator(function*() {
      ...
      var _ref = yield _this2._getTransformOptions(
        entryFiles,
  ...
  transformFile(filePath, transformCodeOptions) {
    var _this3 = this;
    return _asyncToGenerator(function*() {
      const cache = _this3._cache;
      const assetDataPlugins = transformCodeOptions.assetDataPlugins,
        customTransformOptions = transformCodeOptions.customTransformOptions,
        enableBabelRCLookup = transformCodeOptions.enableBabelRCLookup,
        dev = transformCodeOptions.dev,
        hot = transformCodeOptions.hot,
        inlineRequires = transformCodeOptions.inlineRequires,
        isScript = transformCodeOptions.isScript,
        minify = transformCodeOptions.minify,
        platform = transformCodeOptions.platform,
        _projectRoot = transformCodeOptions.projectRoot,
        extra = _objectWithoutProperties(transformCodeOptions, [
          'assetDataPlugins',
      ...
      let fullKey = sha1
        ? Buffer.concat([partialKey, Buffer.from(sha1, 'hex')])
        : null;
      const result = yield fullKey ? cache.get(fullKey) : null;
      console.log('transform', filePath, sha1);
